...
export const createjob = async () => {
  const sql =     `CREATE TABLE IF NOT EXISTS JOB_TABLE (Job_ID VARCHAR PRIMARY KEY, Business_ID INT, User_ID INT, Description VARCHAR(255) NOT NULL,URL VARCHAR(255) NOT NULL,Due_Date DATETIME,FOREIGN KEY (Business_ID) REFERENCES BUSINESS_TABLE(Business_ID), FOREIGN KEY (User_ID) REFERENCES WORKER_TABLE(User_ID));`;

  return executeQuery(sql);
};

export const createjobhistory = async () => {
  const sql =     `CREATE TABLE IF NOT EXISTS JOB_HISTORY (History_ID INT AUTO_INCREMENT PRIMARY KEY,User_ID INT,Job_ID VARCHAR, Business_ID INT, Completion_Date DATETIME,Description VARCHAR(255),Remarks VARCHAR(255),FOREIGN KEY (User_ID) REFERENCES WORKER_TABLE(User_ID), FOREIGN KEY (Business_ID) REFERENCES BUSINESS_TABLE(Business_ID),FOREIGN KEY (Job_ID) REFERENCES JOB_TABLE(Job_ID));`;
  return executeQuery(sql);
};

export const createsubscription_table = async () => {
  const sql =     `CREATE TABLE IF NOT EXISTS SUBSCRIPTION_TABLE (Subscription_ID INT AUTO_INCREMENT PRIMARY KEY,Endpoint VARCHAR(255) NOT NULL,Auth_Key1 VARCHAR(255) NOT NULL,Auth_Key2 VARCHAR(255) NOT NULL,Timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,Job_ID VARCHAR,FOREIGN KEY (Job_ID) REFERENCES JOB_TABLE(Job_ID));`;

  return executeQuery(sql);
};

export const createnotification = async () => {
  const sql =     `CREATE TABLE IF NOT EXISTS NOTIFICATIONS (Notification_ID INT AUTO_INCREMENT PRIMARY KEY,User_ID INT,Job_ID VARCHAR,Notification_Content VARCHAR(255) NOT NULL,Timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,Is_Read BOOLEAN DEFAULT FALSE,FOREIGN KEY (User_ID) REFERENCES WORKER_TABLE(User_ID),FOREIGN KEY (Job_ID) REFERENCES JOB_TABLE(Job_ID));`;
  return executeQuery(sql);
};
