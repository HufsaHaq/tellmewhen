...
export const createsubscription_table = async () => {
  const sql = `CREATE TABLE IF NOT EXISTS SUBSCRIPTION_TABLE (Subscription_ID INT AUTO_INCREMENT PRIMARY KEY, Job_ID VARCHAR(255),Business_ID INT,Endpoint LONGTEXT NOT NULL,Auth_Key1 LONGTEXT NOT NULL,Auth_Key2 LONGTEXT NOT NULL,Timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY (Job_ID) REFERENCES JOB_TABLE(Job_ID), FOREIGN KEY (Business_ID) REFERENCES BUSINESS_TABLE(Business_ID));`;
